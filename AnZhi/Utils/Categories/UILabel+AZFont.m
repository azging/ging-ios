...
//  LinkCity
//
//  Created by 张宗硕 on 8/31/16.
//  Copyright © 2016 张宗硕. All rights reserved.
//

#import "UILabel+AZFont.h"
#import "AZConstant.h"
#import <objc/runtime.h>

@implementation UILabel (AZFont)
/**
...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        ...
        SEL swizzledSelector = @selector(AZBaseInit);
        SEL swizzledSelector2 = @selector(AZBaseInitWithFrame:);
        SEL swizzledSelector3 = @selector(AZBaseAwakeFromNib);
 ...
 * 在这些方法中将你的字体名字换进去
 */
- (instancetype)AZBaseInit {
    id __self = [self AZBaseInit];
    UIFont * font = [UIFont fontWithName:AZFontNameDefault size:self.font.pointSize];
...
- (instancetype)AZBaseInitWithFrame:(CGRect)rect {
    id __self = [self AZBaseInitWithFrame:rect];
    UIFont * font = [UIFont fontWithName:AZFontNameDefault size:self.font.pointSize];
...
- (void)AZBaseAwakeFromNib {
    [self AZBaseAwakeFromNib];
    UIFont * font = [UIFont fontWithName:AZFontNameDefault size:self.font.pointSize];
