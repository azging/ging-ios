...
@interface DXTextMessageToolBar()<UITextViewDelegate, DXFaceDelegate>
{
...
@implementation DXTextMessageToolBar
...
+ (DXTextMessageToolBar *)createToolBar:(CGRect)frame placeHolder:(NSString *)placeHolder delegate:(id<DXTextMessageToolBarDelegate>)delegate {
    DXTextMessageToolBar *inputToolBar = [[DXTextMessageToolBar alloc] initWithFrame:frame];
    inputToolBar.translatesAutoresizingMaskIntoConstraints = YES;
    inputToolBar.autoresizingMask = UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleRightMargin;
    inputToolBar.inputTextView.placeHolder = placeHolder;
    
    inputToolBar.backgroundColor = [UIColor whiteColor];
    inputToolBar.inputTextView.backgroundColor = [AZColorUtil getColor:0xf7f5f3];
    inputToolBar.inputTextView.layer.borderColor = [AZColorUtil getColor:AZColorViewSplitLine].CGColor;
    inputToolBar.inputTextView.layer.borderWidth = 0.5f;
    inputToolBar.inputTextView.layer.cornerRadius = 4.0f;
    inputToolBar.inputTextView.layer.masksToBounds = YES;
    inputToolBar.inputTextView.font = [UIFont fontWithName:AZFontNameDefault size:14];
    inputToolBar.inputTextView.textColor = [AZColorUtil getColor:AZColorFontDefault];
    inputToolBar.delegate = delegate;
    inputToolBar.userInteractionEnabled = YES;
...
//    self.styleChangeButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 0, 0)];
...
//    [self.moreButton setTitle:@"发送" forState:UIControlStateHighlighted];
//    [self.moreButton setTitle:@"发送" forState:UIControlStateSelected];
//    self.moreButton.titleLabel.backgroundColor = [UIColor clearColor];
...
//    self.recordButton.hidden = YES;
    ...
    self.inputTextView.layer.borderColor = [AZColorUtil getColor:AZColorViewSplitLine].CGColor;
...
- (void)buttonAction:(id)sender
{
    ...
    switch (tag) {
        case 0://切换状态
        {
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(didStyleChangeToRecord:)]) {
...
- (void)recordDragOutside
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(didDragOutsideAction:)])
    {
...
- (void)recordDragInside
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(didDragInsideAction:)])
    {
